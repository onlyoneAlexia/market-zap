...
export function registerPortfolioRoutes(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const { marketId, outcomeIndex, txHash } = parsed.data;
      try {
        const tx = await context.deps.settler.rpcProvider.getTransactionByHash(txHash);
        const senderAddress = (tx as any).sender_address ?? (tx as any).contract_address;
        if (senderAddress && senderAddress.toLowerCase() !== address.toLowerCase()) {
          res
            .status(403)
            .json({ error: "Transaction was not sent by the claimed address" });
          return;
        }
      } catch {
        res.status(400).json({
      ...
      await context.deps.db.insertRedemption({
        userAddress: address,
