...
export const TradePanel = React.memo(function TradePanel({ marketId, outcomes, prices, collateralToken, conditionId, onChainMarketId, resolutionTime, resolved, voided, resolvedOutcomeIndex }: TradePanelProps) {
  ...
  const exchangeAvailableRaw =
    balanceData?.available !== undefined ? BigInt(balanceData.available) : undefined;
  const walletBalanceRaw =
    balanceData?.walletBalance !== undefined ? BigInt(balanceData.walletBalance) : undefined;
  ...
  const availableBalance = exchangeAvailableRaw !== undefined
    ? (Number(exchangeAvailableRaw) / 10 ** exchangeDec).toFixed(2)
    : null;
  const walletBalance = walletBalanceRaw !== undefined
    ? (Number(walletBalanceRaw) / 10 ** walletDec).toFixed(2)
    : null;
  const needsDeposit =
  ...
  const marketHasEnded =
    !!resolutionTime && resolutionTime > 0 && resolutionTime <= nowSeconds;
  ...
  const positionQuantity = useMemo(() => {
    ...
    const pos = portfolio.positions.find(
      (p) => p.marketId === marketId && p.outcomeIndex === selectedOutcome,
    );
    if (!pos) return null;
  ...
  const requiredCollateral =
    requiredCollateralRaw !== null
      ? (Number(requiredCollateralRaw) / 10 ** exchangeDec).toFixed(2)
      : null;
  ...
  const { cost, potentialReturn, potentialReturnPositive, takerFee, total } = useMemo(() => {
    ...
    if (!amt) return { cost: "0", takerFee: "0", total: "0", potentialReturn: "0", potentialReturnPositive: true };
  ...
  }, [amount, effectivePrice, orderType]);
  ...
  const handleDeposit = async () => {
    ...
    if (tradingClosed) {
      toast({
        title: marketHasEnded ? "Market ended" : "Trading unavailable",
        description: marketHasEnded
          ? "This market reached its resolution time and can no longer be traded."
    ...
    if (conditionId) {
      try {
        ...
        toast({
          ...
          description: msg.includes("User abort") || msg.includes("rejected")
    ...
    submitOrder.mutate(
      ...
      },
      {
        onSuccess: (data: SubmitOrderResponse) => {
          ...
          if (trades.length === 0 && !order.restingOnBook) {
            ...
            toast({
              ...
              description:
                "Your market order could not be filled. The AMM pool may be exhausted. Try a smaller amount or a limit order.",
              variant: "destructive",
              duration: 7000,
            });
            ...
            const totalFilled = trades.reduce(
              (sum, t) => sum + parseFloat(t.fillAmount),
              0,
            );
            const avgPrice =
              trades.reduce(
                (sum, t) =>
                  sum + parseFloat(t.price) * parseFloat(t.fillAmount),
                0,
              ) / totalFilled;
        ...
        onError: (error: unknown) => {
          // Extract structured error details from MarketZapApiError
          ...
          <>
            ...
            <div className={cn(
              ...
              {outcomes.map((outcome, i) => (
                ...
                >
                  <span className="text-xs font-medium tracking-wide text-muted-foreground truncate max-w-full">
                    {outcome}
                  </span>
                  <span
                    className={cn(
                      "font-mono text-lg font-bold",
                      i === 0 ? "text-yes" : "text-no"
                    )}
                  >
            ...
            >
              ...
              <TabsContent value="limit" className="mt-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-mono font-bold tracking-wider text-muted-foreground">
                    Limit price (%)
                  </label>
                  <Input
                    type="number"
            ...
            <div className="space-y-1">
                ...
                <div className="flex flex-col items-end gap-0.5 text-right">
                  {isConnected && side === "buy" && availableBalance !== null && (
                    <span className="text-xs text-muted-foreground">
                      Deposited: <span className="font-mono">{availableBalance} {tokenSymbol}</span>
                      {needsDeposit && (
                        <span className="ml-1 text-amber">(Wallet: {walletBalance})</span>
                      )}
                    </span>
                  )}
              ...
              <Input
                ...
                className={cn(
                  ...
                  amount && effectiveCap !== null && parseFloat(amount) > effectiveCap && "border-destructive",
                )}
                min={1}
              />
              {amount && effectiveCap !== null && parseFloat(amount) > effectiveCap && (
                <p className="text-xs text-destructive">
                  Exceeds deposited balance. Max ~{Math.floor(effectiveCap * 100) / 100} shares.
              ...
              {side === "buy" && orderType === "market" && requiredCollateral !== null && (
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="font-mono text-foreground">{requiredCollateral} {tokenSymbol}</span>.
            ...
            {amount && parseFloat(amount) > 0 && (
              <div className="space-y-2 text-sm">
                  ...
                  <div className="rounded-md border bg-muted/30 p-2">
                    <div className="mb-1 flex items-center justify-between">
                      ...
                      <span
                        className={cn(
                          "text-xs font-medium",
                          quoteForAmount.quote.canFill ? "text-yes" : "text-amber",
                        )}
                      >
                        {quoteLoading
                          ? "Updating..."
                          : quoteForAmount.quote.canFill
                            ? "Fully Fillable"
                            : "Partial Fill Risk"}
                      </span>
                    </div>
                    <div className="grid gap-2 text-xs sm:grid-cols-2">
                      ...
                      <div className="text-muted-foreground">
                        Est. Avg Price: <span className="font-mono text-foreground">{(Number(quoteForAmount.quote.avgPrice) * 100).toFixed(2)}%</span>
                      </div>
                      <div className="text-muted-foreground">
                        Slippage: <span className="font-mono text-foreground">{(Number(quoteForAmount.quote.slippage) * 100).toFixed(2)}%</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{cost} {tokenSymbol}</span>
                </div>
                <div className="flex justify-between">
                  <TooltipProvider>
                    <Tooltip>
                  ...
                  <span className="font-mono">{takerFee} {tokenSymbol}</span>
                </div>
                <Separator />
                <div className="flex justify-between font-semibold">
                  <span>Total</span>
                  <span className="font-mono">{total} {tokenSymbol}</span>
                </div>
                <div className="flex justify-between font-medium">
                  <span>Potential return</span>
                  <span className={cn("font-mono", potentialReturnPositive ? "text-yes" : "text-no")}>
                    {potentialReturnPositive ? "+" : ""}{potentialReturn} {tokenSymbol}
                  </span>
                </div>
              </div>
            )}
            ...
            <Button
              ...
              disabled={signing || depositing || submitOrder.isPending || (side === "buy" && needsDeposit) || (isConnected && (!amount || parseFloat(amount) <= 0)) || (isConnected && effectiveCap !== null && parseFloat(amount || "0") > effectiveCap)}
            ...
            >
              {signing ? (
                ...
                `${side === "buy" ? "Buy" : "Sell"} ${outcomes[selectedOutcome] ?? "Yes"}`
              )}
            </Button>

            {submitOrder.isError && (
              <p className="text-xs text-destructive">
                {submitOrder.error?.message ?? "Order failed"}
              </p>
            )}
          </>
        )}
